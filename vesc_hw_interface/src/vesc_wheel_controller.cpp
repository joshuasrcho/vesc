...
void VescWheelController::init(hardware_interface::HardwareInfo& info, VescInterface* interface_ptr)
{
  ...
  control_thread_ = std::thread([this]() {
    ...
    while (rclcpp::ok())
    {
      auto next = clock.now() + rclcpp::Duration(0, static_cast<int>(1.0 / this->control_rate_ * 1e9));
      this->control();
      this->interface_ptr_->requestState();
      this->pid_initialize_ = std::fabs(this->target_velocity_) < 0.0001;  // disable PID control when command is 0
